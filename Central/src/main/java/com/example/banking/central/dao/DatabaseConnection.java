...
public class DatabaseConnection {
    ...
    static {
        try {
            ...
            dataSource.setUrl(props.getProperty("db.url").replace("currentaccount_db", "central_db"));
            ...
            logger.info("Pool de connexions MySQL pour Central initialisé avec succès");

        } catch (Exception e) {
            logger.error("Erreur lors de l'initialisation du pool de connexions Central", e);
            throw new RuntimeException("Erreur d'initialisation de la base de données Central", e);
        }
    }
